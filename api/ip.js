...
const { getCorsHeaders, kvRateLimit } = require("./middleware");
...
module.exports = async function handler(req, res) {
  ...
  const reqIp = (req.headers["x-forwarded-for"] || req.headers["x-real-ip"] || "unknown").split(",")[0].trim();
  const limited = await kvRateLimit(`rl:iptool:${reqIp}`, 15, 60).catch(() => false);
  ...
  if (target === "self") target = reqIp.replace(/[^a-fA-F0-9.:]/g, "").slice(0, 45);
  ...
  const safeIp = reqIp.replace(/[^a-fA-F0-9.:]/g, "").slice(0, 45);
  ...
  const VPN_ISP_KEYWORDS = /proton|nordvpn|expressvpn|mullvad|surfshark|privateinternetaccess|pia|cyberghost|ipvanish|purevpn|hidemyass|windscribe|tunnelbear|hotspot.?shield|hide\.me|perfectprivacy|azirevpn|ivpn|airvpn|torguard|vpn\.?ac|fastestvpn|strongvpn|vyprvpn|privatevpn|zenmate|avast.?vpn|norton.?vpn|f-secure|bitdefender.?vpn|kaspersky.?vpn/i;
  const ispForVpn = (d2?.connection?.isp || d2?.isp || d1?.ipAddress || '');
  const orgForVpn = (d2?.org || d2?.connection?.org || '');
  const isVpn       = !!(Number(d1?.isProxy) > 0 || d2?.security?.vpn === true || d2?.security?.vpn === 1 || VPN_ISP_KEYWORDS.test(ispForVpn) || VPN_ISP_KEYWORDS.test(orgForVpn));
